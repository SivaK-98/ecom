...
def get_product_by_id(product_id):
    ...
    try:
        ...
        if product:
            product['rating'] = product.get('rating', None)  # Set rating to None if it doesn't exist
        return product
    except Exception as e:
...
def get_orders_by_username(username=None):
    ...
    try:
        if username:
            ...
            orders = orders_collection.find({"username": {"$regex": username, "$options": "i"}})
        else:
...
def update_order_status(order_id, new_status,description):
    ...
    try:
        result = orders_collection.update_one(
            ...
            {'$set': {'status': new_status,'update_description': description}}
        )
    ...
    pipeline = [
        {
            '$group': {
                '_id': {
                    'year': {'$year': '$date'},  # Replace 'date' with your order date field
                    'month': {'$month': '$date'},
                    'status': '$status'
                },
                'count': {'$sum': 1}
